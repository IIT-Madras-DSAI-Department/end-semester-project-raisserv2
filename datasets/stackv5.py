#removes kmeans features and added svm
import numpy as np
import time
...
from sklearn.svm import LinearSVC # IMPORTED SVM
...
if __name__ == "__main__":
    ...
    print("\n--- Defining the Stacking Ensemble (RF+KNN+LogReg+SVM -> XGB) ---")

    # Level 0: The "Estimators" (Base Models)
    # This is our most diverse stack.
    estimators = [
        ('rf', RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1)),
        ('knn', KNeighborsClassifier(n_neighbors=5, n_jobs=-1)),
        ('logreg', LogisticRegression(multi_class='ovr', solver='liblinear', random_state=42)),
        ('svm', LinearSVC(max_iter=2000, random_state=42, dual=True)) # ADDED SVM
    ]
    ...
    print("\n" + "="*60)
    print(" --- REALISTIC PINNACLE BENCHMARK (PCA-Only Features) ---")
    print("="*60)
    print(f" Base Model (XGBoost Only, on PCA data):")
    print(f"   F1 Score: {f1_base:.6f} | Accuracy: {acc_base:.6f}")
    print(f"\n Stacking Ensemble (RF+KNN+LogReg+SVM -> XGB, on PCA data):")
