# on pca data. using rf+knn+lr as base and xgb as lvl 1
...
if __name__ == "__main__":
    ...
    # We run PCA ONCE at the start to meet the 5-minute time limit.
    ...
    print(f"PCA complete. Selected {pca.n_components_} components.")
    print(f"Data shape after processing: {X_train_processed.shape}")
    

    # --- STAGE 3: Train Base Model (XGBoost Only, on PCA data) ---
    ...
    # Predict on the PROCESSED (PCA) data
    ...
    # Note: 'passthrough=True' means the meta-model *also* gets the PCA'd data.
    estimators = [
    ...
    stacking_model = StackingClassifier(
        ...
        cv=5,  # Use 3 folds for speed. Can increase to 5 for more robustness.
        n_jobs=-1,
        passthrough=True # The meta-model gets preds + the (PCA) original data
    )
    ...
    print("\n" + "="*55)
    print(" --- REALISTIC PINNACLE BENCHMARK (WITH PCA) ---")
    print("="*55)
